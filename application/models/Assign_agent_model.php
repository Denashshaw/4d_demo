<?php
 class Assign_agent_model extends CI_Model
 {
 	...
 	public function upexceldata($data,$client){
		...
		else if($client=='primalbilling'){

       	    if($data[18] != ''){
				$query_res = $this->db->query("SELECT * FROM primalbilling_claims WHERE claim_id='".$data[18]."' ");
				$count = $query_res->result();

				if(count($count) < 1) {
					 $excel_data = array(
		         	  'payer_name'		    		=>$data[0],
		         	  'charge_claim_id'             =>$data[1],
		         	  'facility_address_1'			=>$data[2],
		         	  'patient_id'   				=>$data[3],
		         	  'patient' 		    		=>$data[4],
		         	  'patient_followup'   	   		=>$data[5],
		         	  'charge_from_date'       		=>$data[6],
		         	  'patient_recall'       		=>$data[7],
		         	  'charge_to_date'   	   		=>$data[8],
		         	  'charge_amount' 	       		=>$data[9],
		         	  'charge_status' 	       		=>$data[10],
		         	  'charge_cpt_code' 	       	=>$data[11],
		         	  'charge_first_bill_date' 	   	=>$data[12],
		         	  'charge_last_bill_date' 		=>$data[13],
		         	  'status' 						=>$data[14],
		         	  'date_worked' 	        	=>$data[15],
		         	  'collector' 	        		=>$data[16],
		         	  'claim_note' 	        		=>$data[17],
					...
					$this->db->insert('primalbilling_claims',$excel_data);
				}else{
					$update_data = array(
				  	  'payer_name'		    		=>$data[0],
		         	  'charge_claim_id'             =>$data[1],
					...
					$this->db->where('claim_id', $data[18]);
					$this->db->update('primalbilling_claims',$update_data);
				}
			}
		}
